...
export const Footer = () => {
  ...
  useEffect(() => {
    ...
    gsap.to(brandRef.current, {
      scrollTrigger: {
        trigger: brandRef.current, // elemento que aciona a animação
        start: "top 80%",          // quando 80% do elemento estiver visível
        end: "top 50%",            // até o topo do elemento chegar a 50% da viewport
        scrub: true,               // suaviza a animação conforme o scroll
        markers: true,             // para debugar
      },
      opacity: 1,                  // muda para opacidade total
      duration: 1,
      ease: "power1.out",
    });
  }, []);
